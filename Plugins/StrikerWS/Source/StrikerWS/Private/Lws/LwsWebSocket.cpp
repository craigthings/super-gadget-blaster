...
FLwsReceiveBufferBinary::FLwsReceiveBufferBinary(const uint8* Data, const int32 Size, const int32 InBytesRemaining, const bool IsBinary)
...
int FLwsWebSocket::LwsCallback(lws* Instance, lws_callback_reasons Reason, void* Data, size_t Length)
{
	...
	case LWS_CALLBACK_CLIENT_RECEIVE:
	{
		...
		{
			bWakeGameThread = true;
			ReceiveBinaryQueue.Enqueue(MakeUnique<FLwsReceiveBufferBinary>(static_cast<const uint8*>(Data), Length, BytesLeft, lws_frame_is_binary(Instance) != 0));
		}
		if (bWakeGameThread)
		{
...
void FLwsWebSocket::GameThreadTick()
{
	...
	if (CloseRequest.Code == 0)
	{
		...
		{
			OnRawMessage().Broadcast(BufferBinary->Payload.GetData(), BufferBinary->Payload.Num(), BufferBinary->BytesRemaining, BufferBinary->IsBinary);
		}
	}
}
